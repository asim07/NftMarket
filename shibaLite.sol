...
contract shibaLite is ERC721Full, ReentrancyGuard {
    ...
    */
    ...
    // Buy option
    function buyWithShib(uint256 tokenId,uint _amount) public nonReentrant {

        // is on sale
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, "ERC721Matcha: The collectible is not for sale");

        // transfer funds
        require(token.balanceOf(msg.sender) >= sellBidPrice[tokenId], "ERC721Matcha: Not enough funds");
        ...
        token.transferFrom(msg.sender,ownerOf(tokenId),_amount);
